...
# ij_dataframe = ij_dataframe.round(0)
#r
...
for i in range(0, len(new_ALL_dataframe)):
    ...
    if z<0:
    # if (x>= -R and x<= workpiece_lenght + R) and (y<= workpiece_width + R and y>= -R) and z<0:
    # if (x>= -workpiece_lenght/2 - R and x<= workpiece_lenght/2 + R) and (y<= workpiece_width/ 2 + R and y>= -workpiece_width/ 2 - R) and z<0:
...
# coordinate_path["X"] = 10* (cutting_path["X"] + workpiece_lenght/2 + 2* R)
# coordinate_path["Y"] = 10* (-1* cutting_path["Y"] + workpiece_width/2 + 2* R)
...
for i in range(0, len(movement_dataframe)):
    ...
    previous_y= coordinate_path["Y"][i]
    new_y= coordinate_path["Y"][i+ 1]
# if new_x == previous_x and new_y == previous_y:
...
# Workpiece_array_excel = pd.DataFrame(workpiece_array)
# Workpiece_array_excel.to_excel(excel_writer = r"C:\Users\georg\Desktop\Workpiece.xlsx", header= None, index= False)


# step8 = pd.DataFrame(results_Step["step_4"])
# step8.to_excel(excel_writer = r"C:\Users\georg\Desktop\1.xlsx", header= None, index= False)
# %%
...
def spindle(s):
    if s== 0.0:
        ...
        ps= (-1.18*10**-9)*s**3+(3.44*10**-5)*s**2 - (0.32*s)+1342.63
        return ps
...
file_path = 'VP-6pattern_measuermentpowerdata.xlsx'  
...
plt.ylabel('Power [W]', fontsize=18, fontname='Arial')


plt.legend(loc='upper center', frameon=False, fontsize=18)

# plt.title('VP-6',fontsize=22, fontname='Arial')
