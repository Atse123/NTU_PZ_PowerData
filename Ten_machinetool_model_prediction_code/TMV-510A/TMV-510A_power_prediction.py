...
time_values = Time_all_dataframe.iloc[:26].astype(float)
time_sum = np.floor(time_values.sum().values[0])


Energy_comsumption = (((Time_all_dataframe.astype(float).values[:26,0]*y[:26]).sum()))/(1000)
...
file_path = 'TMV-510Apattern_measuermentpowerdata.xlsx'  
...
plt.title('TMV-510A',fontsize=28, fontname='Arial')
